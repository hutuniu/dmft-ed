        ...
        do i=1,Nbath
           ...
           do io=1,Nspin*Norb
              if(unit_==LOGfile)then
                 if(ed_type=="d") then
                    if(io==1)write(unit_,"(F8.3,a5,90(F8.3,1X))")  real(hybr_aux),"|",(real(hrep_aux(io,jo)),jo=1,Nspin*Norb)
                    if(io/=1)write(unit_,"(a8,a5,90(F8.3,1X))")        "  "      ,"|",(real(hrep_aux(io,jo)),jo=1,Nspin*Norb)
                 endif
                 if(ed_type=="c") then
