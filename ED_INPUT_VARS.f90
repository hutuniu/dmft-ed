...
contains
  ...
  subroutine ed_read_input(INPUTunit)
    ...
    call parse_input_variable(Nspin,"NSPIN",INPUTunit,default=2,comment="Number of spin degeneracy (max 2)")
    ...
    call parse_input_variable(jhflag,"JHFLAG",INPUTunit,default=.true.,comment="Flag to include full SU(2) invariant term: spin-flip, pair-hopping.")
    ...
    call parse_input_variable(lanc_nstates_total,"LANC_NSTATES_TOTAL",INPUTunit,default=1,comment="Initial number of total states to be determined. 1: T=0; >1: T>0")
    ...
    call parse_input_variable(cg_method,"CG_METHOD",INPUTunit,default=1,comment="Conjugate-Gradient method: 0=NR, 1=minimize, 2=CG+.")
    ...
    call parse_input_variable(ed_Type,"ED_TYPE",INPUTunit,default='c',comment="Flag to set real or complex Ham: d=symmetric H (real), c=hermitian H (cmplx)")    
    call parse_input_variable(ed_mode,"ED_MODE",INPUTunit,default='nonsu2',comment="Flag to set ED type: normal=normal, superc=superconductive, nonsu2=broken SU(2)")
    ...
    call parse_input_variable(bath_type,"BATH_TYPE",INPUTunit,default='hybrid',comment="flag to set bath type: normal (1bath/imp), hybrid(1bath)")
