...
function delta_normal_nonsu2(a) result(Delta)
  ...
  do i=1,Ldelta
     do ispin=1,Nspin
        do jspin=1,Nspin
           do ih=1,Nspin
              Delta(ispin,jspin,i) = Delta(ispin,jspin,i) + sum( wps(ispin,ih,:)*wps(ih,jspin,:)/(xi*Xdelta(i) - hps(ih,:)) )
           enddo
        enddo
     enddo
  enddo
...
function g0and_normal_nonsu2(a) result(G0and)
  ...
  do i=1,Ldelta
     fgorb=zero
     zeta = (xi*Xdelta(i)+xmu)*eye(Nspin)
