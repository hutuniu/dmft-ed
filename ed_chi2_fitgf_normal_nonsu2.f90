...
function chi2_delta_normal_nonsu2(a) result(chi2)
  ...
  do l=1,totNspin
     ...
     do i=1,Ldelta
        w = xdelta(i)
        g0(i)   = delta_bath_mats(ispin,jspin,iorb,iorb,xi*w,chi2_bath)
     enddo
     chi2_spin(l) = sum(abs(Gdelta(l,:)-g0(:))**2/Wdelta(:))
  enddo
  !
  chi2=sum(chi2_spin)
  !
...
function grad_chi2_delta_normal_nonsu2(a) result(dchi2)
  ...
  do l=1,totNspin
     ...
     do i=1,Ldelta
        w        = Xdelta(i)
        g0(i)    = delta_bath_mats(ispin,jspin,iorb,iorb,xi*w,chi2_bath)
        dg0(i,:) = grad_delta_bath_mats(ispin,jspin,iorb,iorb,xi*w,chi2_bath,size(a))
     enddo
     !
     do j=1,size(a)
        df(l,j)=&
...
function chi2_weiss_normal_nonsu2(a) result(chi2)
  real(8),dimension(:)         ::  a
  real(8),dimension(totNspin)  :: chi2_orb
  complex(8),dimension(Ldelta) ::  g0
