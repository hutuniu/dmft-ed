program ed_graphene
  ...
  !4x4 matrices for spinful honeycomb Hk:
  complex(8),dimension(4,4)                     :: gamma_0,gamma_x,gamma_y,gamma_z 
  ...
  ! build 4x4 matrix
  gamma_0 = kron_pauli(pauli_0,pauli_0)
  gamma_x = kron_pauli(pauli_x,pauli_0)
  gamma_y = kron_pauli(pauli_y,pauli_0)
  gamma_z = kron_pauli(pauli_z,pauli_0)
  !write(*,*)'gamma_0'
  !do i=1,Nlso
  !   do j=1,Nlso
  !   write(*,*) i, j, gamma_0(i,j)
  !   enddo
  !enddo
  !write(*,*)'gamma_x'
  !do i=1,Nlso
  !   do j=1,Nlso
  !   write(*,*) i, j, gamma_x(i,j)
  !   enddo
  !enddo
  !write(*,*)'gamma_y'
  !do i=1,Nlso
  !   do j=1,Nlso
  !   write(*,*) i, j, gamma_y(i,j)
  !   enddo
  !enddo
  !write(*,*)'gamma_z'
  !do i=1,Nlso
  !   do j=1,Nlso
  !   write(*,*) i, j, gamma_z(i,j)
  !   enddo
  !enddo
  ...
  do while(.not.converged.AND.iloop<nloop)
     ...
     do ilat=1,Nlat
        ...
        Sreal(2,1,1,:,:,:) = Sreal(1,2,2,:,:,:)
        Smats(2,2,2,:,:,:) = Smats(1,1,1,:,:,:)
        Sreal(2,2,2,:,:,:) = Sreal(1,1,1,:,:,:)
     enddo
     ...
     if(Nspin>1)then ! fit also ispin=2
        call ed_chi2_fitgf_lattice(Bath,Weiss,Hloc,ispin=2)
        !! copy bath ineq=1 into ineq=2 with inverted spins to achieve AF 
        !copy_spin_component_bath(Bath(1,:),1,Bath((2,:),2)
        !copy_spin_component_bath(Bath(1,:),2,Bath((2,:),1)
     endif
...
contains
  ...
  function hk_haldane_model(kpoint,Nlso) result(hk)
    ...
    if(Nlso==2)then     ! spinless model
       hk = h0*pauli_0 + hx*pauli_x + hy*pauli_y + hz*pauli_z
    elseif(Nlso==4)then ! spinful model
       hk = h0*gamma_0 + hx*gamma_x + hy*gamma_y + hz*gamma_z
    else 
       stop "hk_haldane_model: Nlso != 2 and Nlso != 4"
    endif
  ...
  subroutine build_hk(file)
    ...
    real(8),dimension(3)                :: b1,b2,b3 ! reciprocal lattice vectors

    !b1=[1/sqrt(3d0),1/3d0,0d0]
    !b2=[0d0,2/3d0,0d0]
    !b3=[0d0,0d0,1d0]
    b1=[3d0/2d0,sqrt(3d0)/2d0,0d0]
    b2=[0d0,sqrt(3d0),0d0]
    b3=[0d0,0d0,1d0]
    ...
    allocate(kxgrid(Nk))
!    kxgrid = kgrid(Nk)
    kxgrid = linspace(0d0,pi2,Nk,iend=.false.) !test [0:,2pi)
!    Hk     = TB_build_model(hk_haldane_model,Nlso,kxgrid,kxgrid,[0d0])
    Hk     = build_hk_model_Norb_c_vect(hk_haldane_model,Nlso,kxgrid,kxgrid,[0d0],b1,b2,b3)
    wtk    = 1d0/Lk
    ...
    if(ED_MPI_ID==0)call write_Hloc(halHloc,'Hloc.txt')
    !
    !
    !Build the local GF:
    ...
    if(Nlso==2)then     ! spinless model
       do ik=1,Lk
          do i=1,Lmats
             Gmats(:,:,i)=Gmats(:,:,i) + inverse_g0k2x2( xi*wm(i)+xmu,Hk(:,:,ik))/Lk
          enddo
          do i=1,Lreal
             Greal(:,:,i)=Greal(:,:,i) + inverse_g0k2x2(dcmplx(wr(i),eps)+xmu,Hk(:,:,ik))/Lk
          enddo
       enddo
    elseif(Nlso==4)then ! spinful model
       do ik=1,Lk
          do i=1,Lmats
             Gmats(:,:,i)=Gmats(:,:,i) + inverse_g0k4x4( xi*wm(i)+xmu,Hk(:,:,ik))/Lk
          enddo
          do i=1,Lreal
             Greal(:,:,i)=Greal(:,:,i) + inverse_g0k4x4(dcmplx(wr(i),eps)+xmu,Hk(:,:,ik))/Lk
          enddo
       enddo
    else 
       stop "build_hk: Nlso != 2 and Nlso != 4"
    endif
    do iorb=1,Nlso
  ...
  subroutine build_hk_path(kpath_)
    ...
    if(present(kpath_))then
       ...
       if(ED_MPI_ID==0)write(LOGfile,*)"Build H(k) along the path G-M-K-G:"
       Npts = 4
       Lk=(Npts-1)*Nkpath
       allocate(kpath(Npts,3))
       kpath(1,:)=kpoint_Gamma
       kpath(2,:)=[1/2d0/sqrt(3d0),1/6d0]*pi2
       kpath(3,:)=[2d0/3d0/sqrt(3d0),0d0]*pi2
       kpath(4,:)=kpoint_Gamma
       file="non_interacting_bands"
    endif
    !
    ...
    if(ED_MPI_ID==0) call TB_Solve_path(hk_haldane_model,Nlso,KPath,Nkpath,&
         ...
         points_name=[character(len=10) :: "G","M","K","G"],&
         file=reg(file))
...
!  !---------------------------------------------------------------------
!  !PURPOSE: obtain a kgrid in tha i-th k direction 
!  !         given the reciprocal basis vectors b1, b2, and b3
!  !---------------------------------------------------------------------
!  function generate_kgrid(kxvect,kyvect,kxvect,ndim,i) result(kigrid)
!    integer,dimension(3)             :: ndim
!    integer                          :: Nkx,Nky,Nkz,Nk,i,ix,iy,iz,ik
!    real(8),dimension(:),allocatable :: kigrid
!
!    Nkgrid=ndim(i)
!    allocate(kigrid(Nkgrid))
!
!    Nkx=ndim(1)
!    Nky=ndim(2)
!    Nkz=ndim(3)
!    Nk =Nkx*Nky*Nkz
!
!    nxgrid = linspace(0d0,1d0,Nkx,iend=.false.) ! [0:1)
!    nygrid = linspace(0d0,1d0,Nky,iend=.false.) ! [0:1)
!    nzgrid = linspace(0d0,1d0,Nkz,iend=.false.) ! [0:1)
!
!    do ik=1,Nk
!       call indx2coord(ik,ix,iy,iz,[Nkx,Nky,Nkz])
!       kigrid = nxgrid(ix)*kxvect(i)+nygrid(iy)*kyvect(i)+nzgrid(iz)*kzvect(i)
!    enddo
!
!    do i=1,ndim(i)
!       kigrid()
...
!  end function generate_kgrid
  ...
  function inverse_g0k2x2(zeta,hk) result(gk)
    integer                     :: i
  ...
  function inverse_g0k4x4(zeta,hk) result(gk)
    integer                     :: i
    ...
    gk=zeta*eye(4)-hk
    call inv(gk)
  end function inverse_g0k4x4



  function build_hk_model_Norb_c_vect(hk_model,Norb,kxgrid,kygrid,kzgrid,xvect,yvect,zvect) result(Hk)
    interface
    ...
    integer                                                                :: ik,ix,iy,iz
    real(8),dimension(3)                                                   :: xvect,yvect,zvect
    ...
    do ik=1,Nk
       call indx2coord(ik,ix,iy,iz,[Nkx,Nky,Nkz])
       kx=kxgrid(ix)*xvect(1)+kygrid(iy)*yvect(1)+kzgrid(iz)*zvect(1)
       ky=kxgrid(ix)*xvect(2)+kygrid(iy)*yvect(2)+kzgrid(iz)*zvect(2)
       kz=kxgrid(ix)*xvect(3)+kygrid(iy)*yvect(3)+kzgrid(iz)*zvect(3)
       Hk(:,:,ik) = hk_model([kx,ky,kz],Norb)
    enddo
  end function build_hk_model_Norb_c_vect
  !
