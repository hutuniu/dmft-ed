...
program ed_slab
  ...
  !<DEBUG
  complex(8),allocatable                :: H_loc_dbg(:,:)              ![Nlat*Norb,Nlat*Norb]
  !DEBUG>
  ...
  real(8),allocatable                   :: epot_site(:),eloc_site(:)    ![Nlat]
  real(8),allocatable                   :: nsite_(:,:),dsite_(:,:)      ![Nindep][Norb]
  real(8),allocatable                   :: epot_site_(:),eloc_site_(:)  ![Nindep]
  !
  ...
  !<DEBUG
  !  try to make like the older scifor square_lattice_dimension
  Lk_ = Nx/2+1
  Lk = Lk_*(Lk_+1)/2
  allocate(epsik(Lk),wt(Lk),kVect(Lk),hk_symm(Lk))
  ik=0
  do ix=0,Nx/2
     do iy=0,ix
        ...
        ky=dble(iy)/dble(Nx)
        !
        kVect(ik)%x=kx*2.d0*pi-pi
        kVect(ik)%y=ky*2.d0*pi-pi
  ...
  allocate(epot_site(Nlat),eloc_site(Nlat))              ![Nlat]
  ...
  allocate(epot_site_(Nindep),eloc_site_(Nindep))           ![Nlat]
  allocate(docc_check_(Nindep))
  !
  allocate(elocal_(Nindep));  Hloc_=0.d0
  do i_ind=1,Nindep
  ...
  allocate(H_loc(Nlat*Norb,Nlat*Norb),H_loc_dbg(Nlat*Norb,Nlat*Norb))
  H_loc=zero
  do ilat=1,Nlat
     do iorb=1,Norb
        do jorb=1,Norb
  ...
  do while(.not.converged.AND.iloop<nloop) 
     ...
     if(symmetry_flag.and.nread==0.d0) then
        ...
        epot_site_ = ed_get_epot_lattice(Nindep)
        eloc_site_ = ed_get_eknot_lattice(Nindep)
...
#ifdef _MPI_INEQ
           call MPI_BCAST(bath,Nlat*Nb,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,mpiERR)
#endif
           call ed_solve_lattice(bath,Hloc,1)
           nsite = ed_get_dens_lattice(Nlat)
           dsite = ed_get_docc_lattice(Nlat)
           epot_site = ed_get_epot_lattice(Nlat)
           eloc_site = ed_get_eknot_lattice(Nlat)
...
contains
  ...
  subroutine print_out_mb(converged)
    ...
    real(8)                                    :: Eint,Ekin,Epot,Eloc_imp,Eloc_mats_sum,Eout(2)
    ...
    if(symmetry_flag) then
       ...
       call store_data("enePotVSisite.data",epot_site_,(/(dble(i),i=1,Nindep)/))
       call store_data("eneLocVSisite.data",eloc_site_,(/(dble(i),i=1,Nindep)/))
    else
       nimp=0.d0
       docc=0.d0
       do ilat=1,Nlat
       ...
       call splot("docc_aveVSiloop.data",iloop,docc_ave,append=.true.)
       call store_data("enePotVSisite.data",epot_site,(/(dble(i),i=1,Nlat)/))
       call store_data("eneLocVSisite.data",eloc_site,(/(dble(i),i=1,Nlat)/))
    end if
    ...
    if(converged)then
       ...
       call store_data("eneLocVSisite.data",epot_site,(/(dble(i),i=1,Nlat)/))
       !
       do iorb=1,Norb
          do jorb=1,Norb
  ...
  subroutine slab_energy
    integer :: unit,ilat,iorb
    !real(8) :: Eint,Ekin,Epot,Eout(2),Eloc
    real(8)                                    :: Eint,Ekin,Epot,Eloc_imp,Eloc_mats_sum,Eout(2)
    real(8) :: Eloc_dbg
    !
    ...
    Ekin = Eout(1)
    !
    Epot=sum(epot_site)/dble(Nlat)
    !
    Eloc_imp=sum(eloc_site)/dble(Nlat)
    Eloc_mats_sum = Eout(2)
    !
    Eint=Ekin+Epot+Eloc_imp    
    unit=free_unit()
    open(unit,file='internal_energy.data')
    write(unit,'(10(F18.10))') Eint,Ekin,Epot,Eloc_imp,Eloc_mats_sum
    close(unit)
    


    ! Eint=Ekin+Eloc+Epot
    ! open(unit,file='test_internal_energy.data')
    ! write(unit,'(10(F18.10))') Eint,Ekin,Eloc,Epot
    ! close(unit)
    ...
    !    do iorb=1,Norb
    !       is = iorb + (ilat-1)*Nlat
    !       Eloc_dbg=Eloc_dbg+nsite(ilat,iorb)*Hloc(ilat,1,1,iorb,iorb)
    !    end do
    ! end do
    ! Eloc_dbg=Eloc_dbg/dble(Nlat)
    ! Eint=Ekin+Eloc_dbg+Epot
    ! open(unit,file='test_internal_energy_dbg.data')
    ! write(unit,'(10(F18.10))') Eint,Ekin,Eloc_dbg,Epot
    ! close(unit)
