program ed_SOC
  ...
  !
  Lfit=min(int((Uloc(1)+1.2+(3*SOC/2))*(beta/pi)),Lmats)
...
contains
  ...
  subroutine Jz_rotate(Fso,type_funct,type_rot,type_freq,bottom_,top_,lvl_)
    ...
    if(type_freq=="wr")then
       ...
       norm=dw
       fact=-1.d0/pi
       if(master)write(LOGfile,'(A11,2(A8,F9.4))') "   real","norm",norm,"fact",fact
    elseif(type_freq=="wm")then
       ...
       norm=1.d0/beta
       fact=1.d0
       if(master)write(LOGfile,'(A11,2(A8,F9.4))') "   imag","norm",norm,"fact",fact
    endif
    !
    !function intake
    do ispin=1,Nspin
