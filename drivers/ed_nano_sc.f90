program ed_nano_sc
  ...
  complex(8),allocatable,dimension(:,:,:,:,:,:,:)   :: Sreal,Sreal_ineq !2*[Nlat*(Nspin*Norb)**2*Lreal]
  ...
  real(8), allocatable,dimension(:)                 :: phi,phi_ineq
  !hamiltonian input:
...
!  call parse_input_variable(chi0ij,"chi0ij",finput,default=.false.)
  ...
  allocate(Weiss_ineq(2,Nineq,Nspin,Nspin,Norb,Norb,Lmats))
  ! allocate self-energy
  allocate(Smats(2,Nlat,Nspin,Nspin,Norb,Norb,Lmats))
  allocate(Smats_ineq(2,Nineq,Nspin,Nspin,Norb,Norb,Lmats))
  allocate(Sreal(2,Nlat,Nspin,Nspin,Norb,Norb,Lreal))
  allocate(Sreal_ineq(2,Nineq,Nspin,Nspin,Norb,Norb,Lreal))
  ! allocate Green's function
  allocate(Gmats(2,Nlat,Nspin,Nspin,Norb,Norb,Lmats))
  allocate(Gmats_ineq(2,Nineq,Nspin,Nspin,Norb,Norb,Lmats))
  allocate(Greal(2,Nlat,Nspin,Nspin,Norb,Norb,Lreal))
  allocate(Greal_ineq(2,Nineq,Nspin,Nspin,Norb,Norb,Lreal))
  ! allocate Hloc
  ...
  ! allocate superconducting order parameter
  allocate(phi(Nlat))
  allocate(phi_ineq(Nineq))
...
!  if(leads) call set_hyb()
  ...
  if(kinetic)then
    ...
    Eout = ed_kinetic_energy_lattice(Hij,[1d0],Smats(1,:,:,:,:,:,:),Smats(2,:,:,:,:,:,:))
    stop
  endif
...
!     allocate(Gijmats(2,Nlat,Nlat,Nspin,Nspin,Norb,Norb,Lmats))
!     allocate(Gijreal(2,Nlat,Nlat,Nspin,Nspin,Norb,Norb,Lreal))
!     if(leads)then
...
!  endif
  ...
  do while(.not.converged.AND.iloop<nloop) 
     ...
     call ed_get_sigma_matsubara_lattice(Smats_ineq(1,:,:,:,:,:,:),Nineq)
     call ed_get_sigma_real_lattice(Sreal_ineq(1,:,:,:,:,:,:),Nineq)
     call ed_get_self_matsubara_lattice(Smats_ineq(2,:,:,:,:,:,:),Nineq)
     call ed_get_self_real_lattice(Sreal_ineq(2,:,:,:,:,:,:),Nineq)
     dens_ineq = ed_get_dens_lattice(Nineq,1)
     docc_ineq = ed_get_docc_lattice(Nineq,1)
     ! get superconducting observables(Nineq,Norb=1)
     phi_ineq = ed_get_phisc_lattice(Nineq,1)
     !  
...
contains
  ...
  subroutine save_sigma(Smats,Sreal)
    ...
    if(size(Smats,4)/=Nspin) stop "save_sigma: error in dim 4. Nspin"
    if(size(Smats,5)/=Norb) stop "save_sigma: error in dim 5. Norb"
    if(size(Smats,6)/=Norb) stop "save_sigma: error in dim 6. Norb"

    if(size(Sreal,3)/=Nspin) stop "save_sigma: error in dim 3. Nspin"
    if(size(Sreal,4)/=Nspin) stop "save_sigma: error in dim 4. Nspin"
    if(size(Sreal,5)/=Norb) stop "save_sigma: error in dim 5. Norb"
    if(size(Sreal,6)/=Norb) stop "save_sigma: error in dim 6. Norb"
    ...
    if(mpiID==0)then
       ...
       do ispin=1,Nspin
          do iorb=1,Norb
             ...
             call store_data("LSigma"//trim(suffix),Smats(1,:,ispin,ispin,iorb,iorb,:),wm)
             suffix="_l"//reg(txtfy(iorb))//"_s"//reg(txtfy(ispin))//"_iw.ed"
             call store_data("LSelf"//trim(suffix),Smats(2,:,ispin,ispin,iorb,iorb,:),wm)
          enddo
       enddo
       do ispin=1,Nspin
          do iorb=1,Norb
             suffix="_l"//reg(txtfy(iorb))//"_s"//reg(txtfy(ispin))//"_realw.ed"
             call store_data("LSigma"//trim(suffix),Sreal(1,:,ispin,ispin,iorb,iorb,:),wr)
             suffix="_l"//reg(txtfy(iorb))//"_s"//reg(txtfy(ispin))//"_realw.ed"
             call store_data("LSelf"//trim(suffix),Sreal(2,:,ispin,ispin,iorb,iorb,:),wr)
          enddo
       enddo
    endif
  ...
  subroutine read_sigma(Smats,Sreal)
    ...
    if(mpiID==0)then
       ...
       do ispin=1,Nspin
          do iorb=1,Norb
             suffix="_l"//reg(txtfy(iorb))//"_s"//reg(txtfy(ispin))//"_iw.ed"
             call read_data("LSigma"//trim(suffix),Smats(1,:,ispin,ispin,iorb,iorb,:),wm)
             suffix="_l"//reg(txtfy(iorb))//"_s"//reg(txtfy(ispin))//"_iw.ed"
             call read_data("LSelf"//trim(suffix),Smats(2,:,ispin,ispin,iorb,iorb,:),wm)
          enddo
       enddo
       do ispin=1,Nspin
          do iorb=1,Norb
             suffix="_l"//reg(txtfy(iorb))//"_s"//reg(txtfy(ispin))//"_realw.ed"
             call read_data("LSigma"//trim(suffix),Sreal(1,:,ispin,ispin,iorb,iorb,:),wr)
             suffix="_l"//reg(txtfy(iorb))//"_s"//reg(txtfy(ispin))//"_realw.ed"
             call read_data("LSelf"//trim(suffix),Sreal(2,:,ispin,ispin,iorb,iorb,:),wr)
          enddo
       enddo
    endif
