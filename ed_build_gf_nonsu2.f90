...
subroutine lanc_build_gf_nonsu2_diagOrb_diagSpin_d(iorb,ispin)
  ...
  do izero=1,numstates
     ...
     if(jsect0/=0)then 
        jdim0  = getdim(jsect0)
        if(ed_verbose<1.AND.ED_MPI_ID==0)write(LOGfile,"(A,2I3,I15)")' add particle:',getn(jsect0),jdim0
     ...
     if(jsect0/=0)then
        jdim0  = getdim(jsect0)
        if(ed_verbose<1.AND.ED_MPI_ID==0)write(LOGfile,"(A,2I3,I15)")' del particle:',getn(jsect0),jdim0
        allocate(HJmap(jdim0),vvinit(jdim0))
...
subroutine lanc_build_gf_nonsu2_mixOrb_diagSpin_d(iorb,ispin,jspin)
  ...
  do izero=1,numstates
     ...
     jsect0 = getCDGsector(ispin,isect0) !this is the same sector I'd get using getCDGsector(JSPIN,isect0)
     if(jsect0/=0)then 
        jdim0  = getdim(jsect0)
        if(ed_verbose<1.AND.ED_MPI_ID==0)write(LOGfile,"(A,2I3,I15)")' add particle:',getn(jsect0),jdim0
        allocate(HJmap(jdim0),vvinit(jdim0))
     ...
     jsect0 = getCsector(ispin,isect0) !this is the same sector I'd get using getCDGsector(JSPIN,isect0)
     if(jsect0/=0)then
        jdim0   = getdim(jsect0)
        if(ed_verbose<1.AND.ED_MPI_ID==0)write(LOGfile,"(A,2I3,I15)")' del particle:',getn(jsect0),jdim0
        allocate(HJmap(jdim0),vvinit(jdim0))
