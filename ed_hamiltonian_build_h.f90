  ...
  case("nonsu2")
  case default
     call push_Whyb_matrix(dmft_bath)
  end select
  ...
  states: do i=first_state,last_state
     ...
     do iorb=1,size(dmft_bath%e,2)
        do kp=1,Nbath
           ...
           htmp = htmp + &
                dmft_bath%e(1,iorb,kp)*dble(ib(ms))       + &
                dmft_bath%e(Nspin,iorb,kp)*dble(ib(ms+Ns))
        enddo
     enddo
     !
     ...
     do iorb=1,Norb
        do kp=1,Nbath
           ...
           !
           !IMP UP <--> BATH UP
           if( (dmft_bath%w(1,1,iorb,kp)/=0d0) .AND. (ib(iorb)==1) .AND. (ib(ms)==0) )then
              call c(iorb,m,k1,sg1)
              call cdg(ms,k1,k2,sg2)
              j = binary_search(Hmap,k2)
              htmp = dmft_bath%w(1,1,iorb,kp)*sg1*sg2
              !
              call sp_insert_element(spH0,htmp,impi,j)
              !
           endif
           !
           if( (dmft_bath%w(1,1,iorb,kp)/=0d0) .AND. (ib(iorb)==0) .AND. (ib(ms)==1) )then
              call c(ms,m,k1,sg1)
              call cdg(iorb,k1,k2,sg2)
              j=binary_search(Hmap,k2)
              htmp = dmft_bath%w(1,1,iorb,kp)*sg1*sg2
           ...
           !IMP DW <--> BATH DW
           if( (dmft_bath%w(Nspin,Nspin,iorb,kp)/=0d0) .AND. (ib(iorb+Ns)==1) .AND. (ib(ms+Ns)==0) )then
              call c(iorb+Ns,m,k1,sg1)
              call cdg(ms+Ns,k1,k2,sg2)
              j=binary_search(Hmap,k2)
              htmp=dmft_bath%w(Nspin,Nspin,iorb,kp)*sg1*sg2
              !
              call sp_insert_element(spH0,htmp,impi,j)
              !
           endif
           !
           if( (dmft_bath%w(Nspin,Nspin,iorb,kp)/=0d0) .AND. (ib(iorb+Ns)==0) .AND. (ib(ms+Ns)==1) )then
           ...
           if(ed_mode=="nonsu2")then
              !IMP UP <--> BATH DW
              if( (dmft_bath%w(1,Nspin,iorb,kp)/=0d0) .AND. (ib(iorb)==1) .AND. (ib(ms+Ns)==0) )then
                 call c(iorb,m,k1,sg1)
                 call cdg(ms+Ns,k1,k2,sg2)
                 j = binary_search(Hmap,k2)
              ...
              if( (dmft_bath%w(1,Nspin,iorb,kp)/=0d0) .AND. (ib(iorb)==0) .AND. (ib(ms+Ns)==1) )then
                 call c(ms+Ns,m,k1,sg1)
                 call cdg(iorb,k1,k2,sg2)
                 j=binary_search(Hmap,k2)
                 htmp = dmft_bath%w(1,Nspin,iorb,kp)*sg1*sg2
              ...
              !IMP DW <--> BATH UP
              if( (dmft_bath%w(Nspin,1,iorb,kp)/=0d0) .AND. (ib(iorb+Ns)==1) .AND. (ib(ms)==0) )then
                 call c(iorb+Ns,m,k1,sg1)
                 call cdg(ms,k1,k2,sg2)
                 j=binary_search(Hmap,k2)
              ...
              if( (dmft_bath%w(Nspin,1,iorb,kp)/=0d0) .AND. (ib(iorb+Ns)==0) .AND. (ib(ms)==1) )then
                 call c(ms,m,k1,sg1)
                 call cdg(iorb+Ns,k1,k2,sg2)
                 j=binary_search(Hmap,k2)
                 htmp=dmft_bath%w(Nspin,1,iorb,kp)*sg1*sg2
