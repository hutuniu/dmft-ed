...
contains
  ...
  !-------------------------------------------------------------------------------------------
#ifdef _MPI
  subroutine ed_solve(mpi_comm,bath_)
...
#ifdef _MPI
    subroutine ed_solve_lattice(mpi_comm,bath,Hloc,iprint,Uloc_ii,Ust_ii,Jh_ii)
      integer           :: mpi_comm
#else
      subroutine ed_solve_lattice(bath,Hloc,iprint,Uloc_ii,Ust_ii,Jh_ii)
        ...
        !<DEBUG
#ifdef _MPI
        MPI_rank=ED_MPI_get_rank(mpi_comm)
        MPI_size=ED_MPI_get_size(mpi_comm)
        MPI_master=ED_MPI_get_master(mpi_comm)
#endif
        !>DEBUG
        !
        ...
        if(MPI_master)then
           do ilat = 1, Nsites
              check_dim = check_bath_dimension(bath(ilat,:))
              if(.not.check_dim) stop "init_lattice_bath error: wrong size(bath,2)"
           end do
        endif
        ...
        if(ED_MPI_colors>MPI_size)ED_MPI_colors=MPI_size
        if(MPI_size<2)ED_MPI_colors=1
        if(MPI_master)print*,"MPI_SIZE      =",MPI_size
        if(MPI_master)print*,"MPI_COLORS    =",ED_MPI_colors
        if(MPI_master)print*,"MPI_SIZE_COLOR=",MPI_size/ED_MPI_colors
        !
        MPI_color=mod(MPI_rank,ED_MPI_colors)
        !
        call MPI_Comm_split(mpi_comm,MPI_color,0,MPI_color_comm,MPI_ierr)
        mpi_color_rank=MPI_Get_rank(MPI_color_comm)
        mpi_color_size=MPI_Get_size(MPI_color_comm)
        write(LOGfile,*) "Global ", MPI_rank," is now local rank ",MPI_color_rank," of color: ",MPI_color
        call MPI_Barrier(MPI_color_comm,MPI_ierr)
        if(MPI_master)print*,""
        !<DEBUG
        !
        !
        if(MPI_master)call start_timer
        ! if(MPI_master)LOGfile = 800+MPI_rank
>>> HERE at 06Dec2015
        do ilat = 1+MPI_color_rank, Nsites, MPI_color_size
           write(tmp_suffix,'(I4.4)') ilat
           ed_file_suffix="_site"//trim(tmp_suffix)
           !
