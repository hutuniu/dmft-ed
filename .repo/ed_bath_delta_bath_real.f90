...
function delta_bath_real_main(x,dmft_bath_) result(Delta)
  ...
  case default
     ...
     case ("nonsu2")
        ...
        ! !                    \sum_k [ U_{a}^{s}(k)*U_{a}^{s}(k)/((w+i\h - E_{a}^{sbar}(k)) ]
        ! !
        ! !\Delta_{aa}^{ss`} = \sum_k [ V_{a}^{s}(k)*U_{a}^{s`}(k)/((w+i\h - E_{a}^{s}(k)) ] + 
        ! !                    \sum_k [ U_{a}^{s}(k)*V_{a}^{s`}(k)/((w+i\h - E_{a}^{sbar}(k)) ]
        ! do iorb=1,Norb
  ...
  case ("hybrid")             !hybrid: all _{ab} components allowed (inter-orbital local mixing present)
     ...
     case ("nonsu2")
        ...
        ! !                    \sum_k [ U_{a}^{s}(k)*U_{b}^{s}(k)/(w+i\h - E^{sbar}(k)) ]
        ! !
        ! !\Delta_{ab}^{ss`} = \sum_k [ V_{a}^{s}(k)*U_{b}^{s`}(k)/(w+i\h - E^{s}(k)) ] + 
        ! !                    \sum_k [ U_{a}^{s}(k)*V_{b}^{s`}(k)/(w+i\h - E^{sbar}(k)) ]
