...
contains
  ...
  subroutine search_chempot(xmu_tmp,dens_tmp,converged_)!,write_6)
    ...
    if ((dabs(diffdens)).le.nerr) then
       ...
       if (ilarge*ismall.eq.0) then
         ...
         ! if (inotbound>=4)  delta_xmu = 0.4d0 !delta_xmu*2.0d0
          if (inotbound>=8)  delta_xmu = 0.5d0 !delta_xmu*2.0d0
  ...
  subroutine SOC_jz_symmetrize(funct)
    ...
    do ifreq=1,Lfreq
       funct_out(:,:,ifreq)=matmul(transpose(conjg(atomic_SOC_rotation())),matmul(funct_in(:,:,ifreq),atomic_SOC_rotation()))
    enddo
    !
    ...
    funct_in=zero
    do ifreq=1,Lfreq
       funct_in(:,:,ifreq)=matmul(atomic_SOC_rotation(),matmul(funct_out(:,:,ifreq),transpose(conjg(atomic_SOC_rotation()))))
    enddo
    !
    !founction out
    funct=zero
