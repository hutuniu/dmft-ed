...
MODULE ED_GREENS_FUNCTIONS
  ...
  USE SF_LINALG,  only: matrix_inverse,inv
  !<debug
  USE DMFT_FFTGF
  !>debug
  USE PLAIN_LANCZOS
...
contains
  ...
  subroutine buildchi_impurity()
    ...
    !
    !
    if(.not.allocated(densChi_tau))allocate(densChi_tau(Norb+1,0:Ltau))
    if(.not.allocated(densChi_w))  allocate(densChi_w(Norb+1,Lreal))
    if(.not.allocated(densChi_iv)) allocate(densChi_iv(Norb+1,0:Lmats))
    densChi_tau=zero
    densChi_w=zero
    densChi_iv=zero
    !
